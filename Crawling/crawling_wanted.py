...
class Wanted(chromedriver):
    ...
    # 링크 데이터 크롤링 함수
    def getlink(self):
        # 컬럼 사전으로부터 데이터 크롤링 시도
        for c_name, p_name in self.columns.items():
            self.driver.get(self.URL)                       # 원티드 사이트 개발자 주소
            self.driver.implicitly_wait(time_to_wait=60)    # 사이트 갱신 대기
            try:                # 목록 창
                dr = self.driver.find_element
                (By.CLASS_NAME, "JobCategory_JobCategory__btn__k3EFe")
                dr.click()      # 목록창 열기
                time.sleep(1)   # 잠깐 대기
            ...
            for position in p_name:             # 사전 value 값 가져오기
                ...
                for i in self.driver.find_elements(By.TAG_NAME, "button"):
                    if i.text == position:      # 사전 value 값과 비교
                        i.click()               # 같다면 선택하기
            # 조건 검색하기 버튼
            tg = self.driver.find_element(
                By.CLASS_NAME, "Button_Button__label__1Kk0v").find_element(
                    By.XPATH, "..")
    ...
    def scroll(self, pos):
        ...
        for item in div:    # 데이터 순회
            ...
            tmp = pd.DataFrame(
                [[pos, link, 'F']], columns=super().getlinkcol())   # 임시 데이터프레임
            self.linkdf = pd.concat(
                [self.linkdf, tmp], ignore_index=True)  # 데이터프레임 합치기
        ...
        for pos, URL in zip(self.linkdf['position'], self.linkdf['link']):
            ...
            try:                        # 요소가 있는지 검사, 있다면 텍스트 데이터 가져오기
                data = self.driver.find_element(
                    By.CLASS_NAME, "JobContent_descriptionWrapper__SM4UD").text

            except BaseException:       # 만약 요소가 없다면
                data = None             # 빈 데이터를 반환

            tmp = pd.DataFrame(
                [[pos, data, 'F']], columns=super().getdatacol())   # 임시 데이터프레임
            self.datadf = pd.concat(
                [self.datadf, tmp], ignore_index=True)  # 데이터 합치기
        return self.datadf
