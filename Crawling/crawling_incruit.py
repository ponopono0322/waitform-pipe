...
class Incruit(chromedriver):
    ...
    def getlink(self):
        for c_name, p_name in self.columns.items():     # 컬럼 사전으로부터 데이터 크롤링 시도
            ...
            for position in p_name:             # 사전 value 값 가져오기
                # 버튼 태그 목록 가져와서
                for i in self.driver.find_elements(By.TAG_NAME, "span"):
            ...
            time.sleep(1)
            # 조건 검색하기 버튼
            sh = self.driver.find_element(By.ID, "SearchResultCount")
    ...
    def isnext(self):
        ...
        except BaseException:   # 요소가 없으면 False 반환
            n_btn = False
    ...
    def nextpage(self, pos):
        ...
        for item in self.driver.find_elements(By.CLASS_NAME, "c_row"):
            ...
            li = item.find_element(
                By.CLASS_NAME, "c_col").find_element(
                    By.CLASS_NAME, "cell_mid").find_element(
                        By.CLASS_NAME, "cl_top").find_element(By.TAG_NAME, "a")
            # 임시 데이터프레임 저장
            txt = li.get_attribute('href')
            tmp = pd.DataFrame([[pos, txt, 'F']], columns=super().getlinkcol())
            # 데이터 저장
        ...
        for pos, URL in zip(self.linkdf['position'], self.linkdf['link']):
            ...
            try:
                content = self.driver.find_element(
                    By.TAG_NAME, "iframe")              # 프레임 요소
                self.driver.switch_to.frame(content)    # 프레임 전환
                # 텍스트 데이터 가져오기
                data = self.driver.find_element(By.ID, "content_job").text
            except BaseException:       # 만약 요소가 없다면
                data = None             # 빈 데이터를 반환
            # 임시 데이터프레임
            tmp = pd.DataFrame(
                [[pos, data, 'F']], columns=super().getdatacol())
            # 데이터 합치기
