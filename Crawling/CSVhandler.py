...
class CSVhandler:
    ...
    def searchnewdata(self, path, before_df, type='data'):
        ...
        if self.filecheck(path):        # 경로에 기존 파일이 있다면
            new_df = pd.concat(
                [pd.read_csv(path), before_df], ignore_index=True)
    ...
    def csvcompress(self, path):
        ...
        if self.filecheck(path):        # csv 파일이 존재하는지 검사
            ...
            # 압축할 파일 이름
            csv_zip.write(path, compress_type=zipfile.ZIP_DEFLATED)
            csv_zip.close()             # 파일 닫기
    ...
    def newdata(self, input_df, type):
        # 중복 제거 후
        conv_df = input_df.drop_duplicates(
            ['position', type], keep='first', ignore_index=True)
